...
{
    public partial class Item_Manage : EmaxBasepage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            //   csText.Append("<script type=\"text/javascript\"> function loadaccordion() {");
            //    csText.Append(@"$('#accordion').accordion({heightStyle: 'content'}); };");
            //    csText.Append(@"</script>");
            //    cs.RegisterClientScriptBlock(csType, csName, csText.ToString(),false);
            ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), this.pageid, "<script>$(function () {$('#accordion').accordion({ heightStyle: 'content',  collapsible: true, active: false});})</script>", false);
            //}
            if (!IsPostBack)
            
            {
            
            //  ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "loadAccordion", "loadaccordion()",false);
           
                try
                {
                    string autoitem = SqlCommandHelper.GetTokenKey("autoitem", Request.Cookies["Token"].Value);
                    if(autoitem=="False")
                    {
                    ...
                    hf_sysvatvalue.Value = SqlCommandHelper.GetTokenKey("vat", Request.Cookies["Token"].Value);
                    if (Request.QueryString["itemid"] != null && Request.QueryString["itemid"] != string.Empty)
                    {
                        hf_itemid.Value = Request.QueryString["itemid"];
                        Dictionary<object, object> dict = new Dictionary<object, object>();
           ...
           { txt_itemcode,txt_itemcode2,txt_itemcode3,txt_itembarcode,txt_itembarcode2,txt_itemname,txt_itemename,txt_itemdesc,hf_itemid,cmb_unitid,cmb_groupid,cmb_itemtypeid,
            ...
            if (res.errorid == 0)
            {
                if (res.outputparams != null && res.outputparams.Count > 0)
                {
                    ...
                    if (res.outputparams.ContainsKey("itemcode")){ txt_itemcode.Value =EmaxGlobals.NullToEmpty(res.outputparams["itemcode"]); }
        ...
        {
            if (!Directory.Exists(Server.MapPath("~/Img/Item/")))
            {
                Directory.CreateDirectory(Server.MapPath("~/FImgiles/Item/"));
            }
            if (e.IsValid)
            {
        ...
        {
            SqlCommandHelper.GetTokenKey("vat", Request.Cookies["Token"].Value);
        }
    }
}