...
{
    public partial class S_order : EmaxBasepage
    {
                ...
                try
                    {
                        if (Request.QueryString["orderno"] != null && Request.QueryString["orderno"] != string.Empty)
                        {
                            ...
                            var res = SqlCommandHelper.ExcecuteToDataTable("s_order_sel_search", dict).dataTable;
        ...
        protected void btn_Save_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                var res = SaveData(EmaxGlobals.NullToIntZero(HF_soid.Value) == 0 ? "s_order_ins" : "s_order_upd"
        , GetParam(), null,
        ...
        {
            var res = SaveData(EmaxGlobals.NullToIntZero(HF_sodtlid.Value) == 0 ? "s_orderdtls_ins" : "s_orderdtls_upd"
, GetParam_dtl(), null,
  null, true, true,
  new List<ParamObject>() { new ParamObject() { ParamName = "branchname", ParamValue =cmb_branchid } });
            if (res.errorid == 0)
        ...
        DataTable GvdetailSource()
        {
            ...
            return SqlCommandHelper.ExcecuteToDataTable("s_orderdtls_sel", dict).dataTable;
        ...
        protected void btn_attach_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var res = SaveData("s_order_itemexcel_ins", new List<object> { HF_soid }, null,
                null, true, true, null
                , null);
                if (res.errorid == 0)
                {
        ...
        protected void btn_xlsxexport_Click(object sender, EventArgs e)
        {
            try
            {
                ExportingDevExpressUtil.Export(gvitemsExporter, "اصناف أمر البيع", 1, Request.GetOwinContext().Request.User.Identity.Name, gvs_orderdtls.GetSelectedFieldValues("sodtlid").Count != 0, false, "أصناف أمر البيع رقم " + txt_sono.Text);
            }
            catch (Exception ex)
            {
        ...
        protected void btn_createinv_Click(object sender, EventArgs e)
        {
            try
            {
                var res = SaveData("s_order_to_inv_ins", new List<object> { HF_soid, cmb_branchid }, null, new List<string>() { "sinvc" }, true, false, null);
                if (res.errorid == 0)
                {
                    ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "mykey", "sweetsuccess('" + res.errormsg + "');", true);
                    disable();
        ...
        protected void btn_print_Click(object sender, EventArgs e)
        {
            ...
            PrintPage("Sales/s_order_page.repx", dict);
        }
    }
}